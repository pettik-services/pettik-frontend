...
const GroomingReviewElement: React.FC<Props> = ({
  values,
  setValues,
  handleBookGrooming,
}) => {
  ...
  return (
    <div className='flex flex-col gap-y-6 py-6'>
      <div>
        ...
        <div>Review the booking details and Place the order now done</div>
      </div>
      <div className='flex'>
        <div className='flex flex-col bg-grey-darker rounded-2xl items-center p-6 justify-center text-center gap-y-4'>
          ...
          <div className='font-bold text-xl'>{values?.service?.name}</div>
          <div>
          ...
          <div className='flex justify-between w-full font-bold px-4 py-6'>
            ...
            <div className='text-green-700'>{values?.service?.cost}</div>
          </div>
          <Button
            variant='contained'
...
export default GroomingReviewElement;
